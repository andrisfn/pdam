...
<body>
<?php 
    if ($this->session->flashdata('pesanLogin')) {
       if($this->session->flashdata('kondisi')=='0'){
    ?>
            <script>
                ...
                Toast.fire({
                    icon: 'error',
                    title: '<?= $this->session->flashdata('pesanLogin') ?>'
                })
            </script>
    <?php
        }else{
		...
		<div class="container-login100">
			<div class="wrap-login100">
				...
				<form class="login100-form validate-form" action="<?= base_url('login/aksi_login') ?>" method="post">
					<div class="wrap-input100 validate-input m-b-26" data-validate="Username diibutuhkan">
					...
					<div class="container-login100-form-btn mt-4">
						Belum punya akun ? <a href="<?= base_url('login/daftar') ?>" class="ml-1"> <u> Daftar disini </u></a>
					</div>
				</form>
			</div>
		</div>
